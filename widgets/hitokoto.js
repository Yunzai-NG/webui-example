 ...
 * 注意事项：**这枚组件示范的是「组件自己的交互」与「包自带样式表」。** 面板上多数组件是只读的
 *          （采样来了就画），而这一枚有两个按钮：一个要发请求、要转圈、可能失败，一个要写剪贴板。
 *
 *          **取数走包自己的 node 侧（`api.own`），不在浏览器里直连接口。** 浏览器直连要看对方的
 *          CORS 头，而那不是我们能控制的东西；且接口地址存在配置里，由 node 侧读同一份配置发请求，
 *          浏览器就不必知道那个地址长什么样。
 *
 *          **失败时保留上一句话。** 一次网络失败把已经显示着的句子换成一行错误，等于「点一下就
 *          丢了内容」；故错误另起一行小字，句子留在原处。
 *
 *          **排版用本包自带的 `style.css`**（`quote-*` 一族），不用面板的 `.card`：这一枚要的是
 *          居中斜体加一枚大引号，而面板的卡片类是左对齐的通用容器。
 */

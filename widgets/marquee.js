 ...
 * 注意事项：**这枚组件示范的是「自带一份 .css」。** 横向滚动要一段 `@keyframes` 与一处
 *          `overflow: hidden`，写成行内样式做不到 —— 行内写不了关键帧，也写不了伪元素。
 *          故本包在 `package.json` 里声明了 `webuiPanel.style`，面板会把那份 css 每条选择器
 *          限定到本包之后注入。
 *
 *          **类名故意取得很泛（`.marquee`、`.track`）。** 它们撞不到面板自身，也撞不到别的
 *          包 —— 限定之后实际生效的是 `[data-panel="panels/webui-example"] .track`。这正是
 *          自带样式表值得用的理由：不必给每个类名手工加前缀。
 *
 *          **动画名反而必须加前缀（`example-marquee-roll`）。** `@keyframes` 的名字是全局的，
 *          面板不会去改它；两个包各写一段叫 `roll` 的动画时，后注入的那份会盖掉前一份，
 *          而表现是「另一个插件的动画忽然变了速度」，极难查。
 *
 *          **颜色一律取 `var(--…)`。** 自带样式表不改变这条口径：写死的色值在另一套主题下
 *          会看不见，而变量的取值由主题决定。
 */

 ...
 * 注意事项：示范「一枚组件带自己的样式表」：月历是 7 列网格，面板样式表里没有，故本包自带的
 *          `style.css` 里有 `.cal-*` 一族，注入前会被面板限定到本包之后。
 *          节拍是分钟级（这一枚上没有秒），故自带计时器而不用 `api.onTick`（那是采样用的 5 秒一拍）。
 *          计时器整个换一个新 `now`，不只更新「今天还剩多少」—— 否则面板开着过夜，第二天看到的还是昨天。
 *          农历取不到时只少几行，不让整枚组件报错；春节那块与农历同源，故一并退场。
 */
 ...
 * 一段区间还剩几天，**不含今天**
 *
 * 三条进度共用这一个口径，不各自写一遍减法 —— 各写一遍时漏掉减 1 的那条会比其余多算一天，
 * 而三个数只有并排看才露出来，它们恰好就是并排的。不含今天：算上会让周日显示「本周还剩1天」。
 * @param at 当前时刻
